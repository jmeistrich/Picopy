  ...
  var Live = {
    ...
    loadresources: function () {
      ...
      for (var i = 0; i < links.length && active.css; i++) {
        ...
        if (href && rel && rel.match(new RegExp("stylesheet", "i")) && isLocal(href) && href.indexOf("data") != 0) {
          uris.push(href);
    ...
    refreshJs: function(file) {
        ...
        for (var i = 0; i < scripts.length; i++) {
            if (scripts[i].src.indexOf(file) != -1) {
                newScript.src = file + "?jsreload=" + new Date().getTime();
                newScript.innerHTML = scripts[i].innerHTML;
                removeScript = scripts[i];
                break;
            }
        }
        head.removeChild(removeScript);
